...
int main(int argc, char *argv[]) {
    ...
    QCommandLineOption modelOption(QStringList() << "m" << "model", "Gemini model name (default: gemini-2.0-flash)", "model", "gemini-2.0-flash");
    ...
    if (apiKey.isEmpty() && QFile::exists(configPath)) {
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            if (!doc.isNull() && doc.isObject()) {
                ...
                if (model == "gemini-2.0-flash" && obj.contains("gemini_model") && !obj.value("gemini_model").toString().trimmed().isEmpty()) {
