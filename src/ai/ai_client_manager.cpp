...
void AIClientManager::loadBlacklist() {
    ...
    QString blacklistPath = ConfigUtils::resolveConfigFilePath("blacklist.txt");
...
void AIClientManager::loadWhitelist() {
    ...
    QString whitelistPath = ConfigUtils::resolveConfigFilePath("whitelist.txt");
...
void AIClientManager::loadUserNames() {
    ...
    QString path = ConfigUtils::resolveConfigFilePath("user_names.json");
        ...
        if (!userData.isEmpty()) {
            ...
            if (!preferred.isEmpty()) {
                ...
                systemInstructions = QString(
                    "[システム指示: このコメントの投稿者は「%1」さんです。回答の冒頭で，必ず「%1さん、」または「%1、」と呼びかけてください。他の呼び方は使わず、この呼び方で統一してください。また、もし今回のコメントで発言者自身から新たな呼び方の変更指示（例：「〇〇です」などの自己紹介や「〇〇と呼んで」などの指示）があった場合のみ、その指示に従い、今後の対話でそれを反映させてください。文脈中の第三者や配信者の名前をニックネーム設定と誤認しないでください。]"
                ).arg(preferred);
            } else if (!nicknames.isEmpty()) {
                // 愛称リストがある場合
                QString nicknamesStr = nicknames.join("、");
                systemInstructions = QString(
                    "[システム指示: このコメントの投稿者の%1アカウント名は「%2」です。愛称（呼び名）の候補は「%3」です。回答の冒頭で、これらの愛称候補からいずれか1つをランダムに選んで『〇〇さん、』や『〇〇ちゃん、』などと呼びかけて回答してください。また、もし今回のコメント内で発言者自身から「〇〇と呼んで」のような呼び方の指定・変更指示、あるいは「〇〇です」といった自己紹介があった場合のみ、その指示した呼び方を最優先で使用し、今後の回答でもその呼び方を使用してください。文脈中の第三者や配信者の名前をニックネーム設定と誤認しないでください。]"
                ).arg(platformName).arg(cleanUser).arg(nicknamesStr);
            } else {
                // 登録はあるが愛称リストも優先呼び名も空の場合 (アカウント名そのまま)
                systemInstructions = QString(
                    "[システム指示: このコメントの投稿者の%1アカウント名は「%2」です。回答冒頭での強制的な名前の呼びかけ（「%2さん、」等）は行わず、日本語として自然な文章（例:『それは〇〇だよ！』『〇〇です』等）で直接回答してください（自然な省略応答を許可します）。また、本チャットは不特定多数の視聴者が参加するマルチユーザー環境です。会話履歴やセッション要約に過去の他ユーザー名が含まれていても、過去の会話相手と混同せず、現在の発言者（%2）に対して適切かつ独立して回答してください。もし今回のコメントで発言者自身から別の呼び方の指示や「〇〇です」などの自己紹介があった場合のみ、その指示した呼び方を使用してください。文脈中の第三者や配信者の名前をニックネーム設定と誤認しないでください。]"
                ).arg(platformName).arg(cleanUser);
            }
        } else {
            // 未登録ユーザーの場合：冒頭での強制呼びかけを行わず自然な文章で回答
            systemInstructions = QString(
                "[システム指示: このコメントの投稿者の%1アカウント名は「%2」です。回答冒頭での強制的な名前の呼びかけ（「%2さん、」等）は行わず、日本語として自然な文章（例:『それは〇〇だよ！』『〇〇です』等）で直接回答してください（自然な省略応答を許可します）。また、本チャットは不特定多数の視聴者が参加するマルチユーザー環境です。会話履歴やセッション要約に過去の他ユーザー名が含まれていても、過去の会話相手と混同せず、現在の発言者（%2）に対して適切かつ独立して回答してください。もし今回のコメント内で発言者自身から「〇〇と呼んで」などの呼び方の指定・変更指示、または「〇〇です」といった自己紹介があった場合のみ、その指示した呼び方を使用してください。文脈中の第三者や配信者の名前をニックネーム設定と誤認しないでください。]"
            ).arg(platformName).arg(cleanUser);
        }
