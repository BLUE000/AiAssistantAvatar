    ...
    if (!cleanUser.isEmpty() && !isSystemGreeting) {
        ...
        if (!userData.isEmpty()) {
            ...
            if (!preferred.isEmpty()) {
                ...
                systemInstructions = QString(
                    "[システム指示: このコメントの投稿者の%1アカウント名は「%2」です。回答冒頭での強制的な名前の呼びかけ（「%2さん、」等）は行わず、日本語として自然な文章（例:『それは〇〇だよ！』『〇〇です』等）で直接回答してください（自然な省略応答を許可します）。また、本チャットは不特定多数の視聴者が参加するマルチユーザー環境です。会話履歴やセッション要約に過去の他ユーザー名が含まれていても、過去の会話相手と混同せず、現在の発言者（%2）に対して適切かつ独立して回答してください。もし今回のコメントで別の呼び方の指示や「〇〇です」などの自己紹介があれば、その指示した呼び方を使用してください。]"
                ).arg(platformName).arg(cleanUser);
            }
        } else {
            // 未登録ユーザーの場合：冒頭での強制呼びかけを行わず自然な文章で回答
            systemInstructions = QString(
                "[システム指示: このコメントの投稿者の%1アカウント名は「%2」です。回答冒頭での強制的な名前の呼びかけ（「%2さん、」等）は行わず、日本語として自然な文章（例:『それは〇〇だよ！』『〇〇です』等）で直接回答してください（自然な省略応答を許可します）。また、本チャットは不特定多数の視聴者が参加するマルチユーザー環境です。会話履歴やセッション要約に過去の他ユーザー名が含まれていても、過去の会話相手と混同せず、現在の発言者（%2）に対して適切かつ独立して回答してください。もし今回のコメント内で「〇〇と呼んで」などの呼び方の指定・変更指示、または「〇〇です」といった自己紹介があった場合は、その指示した呼び方を使用してください。]"
            ).arg(platformName).arg(cleanUser);
        }
