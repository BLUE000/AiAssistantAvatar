...
namespace AIRandomUtils {
    ...
    /**
     * @brief シード文字列（日付やユーザー名等）に基づき、min から max の閉区間 [min, max] で決定論的な疑似乱数を算出する。
     */
    ...
    /**
     * @brief 入力テキスト内に含まれる "Random(min, max)" や "DailyRandom(min, max, seed)", "RandomList(max, count)" マクロ式を自動解釈・評価し、置換文字列を返す。
     */
