...
6. **Auto AI Provider Selector (全解除時自動選定部)**: UI で全プロバイダのチェックが外されている場合、Release ビルドではダミーを稼働させず、`RateLimitTracker` から現在利用可能かつリミット残数（RPM/RPD）が最大の最適な AI クライアントを自動選定してルーティングする。なお、有効な API キーが 1 つも設定されていない場合は「APIキー設定案内」を表示する。設定済みプロバイダが全てレートリミット到達時は、未設定キーの有無に応じた二分岐案内（リセットまでの残分秒 ＋ 未設定プロバイダの追加登録案内）を生成・表示する。
...
1. **1段目：`AIClientManager`（スレッド窓口・共通管理 ＆ ルーティング）**
   - コアスレッドからの要求をスロット（`on_requestAI`）で受信。コアモジュールからは常にこのクラスのみが見える。
   - ユーザーがUIで明示的に選択した AI プロバイダを最優先とし、他プロバイダへの勝手なすり替え（サイレントフォールバック）を禁止する。選択されたプロバイダ自身のエラーを正確にキャプチャして通知する。
   ...
   - APIキーの管理や、タイムアウト制御、およびシステム固定自動応答（F-21）における「使用中AI/モデル問い合わせ」に対してプロバイダ名に加えアクティブな動的モデル名 (`currentModelName`) を組み込んだ回答テキスト生成などの共通処理をここで吸収する。

2. **2段目：`IAIClient` 具象クラス（各AI API固有の個別処理）**
   - HTTPのヘッダーおよびペイロード（JSON）の構築。
   - `HuggingFaceAIClient`: OpenAI 互換のエンドポイント (`https://router.huggingface.co/v1/chat/completions`) の構築、`/v1/models` による利用可能モデルの全自動取得・動的選出、および詳細エラーレスポンスのパース。
   - `OpenRouterAIClient`: OpenAI 互換エンドポイント (`https://openrouter.ai/api/v1/chat/completions`) の構築、`/v1/models` による `:free` 無料枠・最適モデルの全自動取得・動的選出、および詳細エラーパース。
...
6. **UIアーキテクチャ・共通設定 ＆ OBS用URL化 (F-30)**
   ...
   - 「Twitch 連携設定」グループボックスをスリム化（チャンネル名および起動時挨拶のみ。クライアントIDとOAuth用ポートは設定ファイル `local_settings.json` のみに保持しUIからは削除）。

7. **TaskFlow(予定管理システム) 独立連携 ＆ 全プラットフォーム拡張設計 (F-31)**
   - 「TaskFlow(予定管理システム)連携設定」グループボックスを新規独立配置し、有効化チェックボックスと任意可変 API URL 設定項目を設ける。
...
9. **会話履歴ビューア ＆ 右クリックメニュー廃止 (F-30)**
   - **右クリックメニューの廃止**: 不定形透過ウィンドウ時代の右クリックメニュー（`contextMenuEvent`）を完全に削除し、操作をメインUI・タスクトレイ・設定ダイアログへ一元化。
   - **会話履歴ダイアログ (`HistoryViewerDialog`) の新設**:
     - 設定画面から「会話履歴を表示」ボタンを押すことでモーダル起動。
     - **ページネーション (10/50/100件)**: プルダウン切り替えと前後ボタンによる高速ページ切り替え。UIスレッドのフリーズを完全防止。
     - **サマリ化状態表示**: `[サマリ化済]` / `[未サマリ]` の表示タグおよび上部ステータス表示。
     - **平文エクスポート (.txt)**: UI描画を介さない直接テキストファイル出力。
     - **手動強制サマリ化**: `AIClientManager` の `forceSummarizeHistory()` を発火して即座に会話ログを要約圧縮。
   - `AIClientManager` にて既存のプロバイダと並列して登録・管理し、レートリミットトラッカー (`RateLimitTracker`) や自動フォールバック機構、AIルーティング (`AIRouter`) への組み込みを行う。
   - UI 設定ダイアログに HuggingFace, OpenRouter, さくらAI のプロバイダ選択オプションおよび API キー入力欄を追加（全自動モデル選出プロバイダは API キー入力欄のみの直感的なレイアウト構造）。
...
#### F-34 既存テーブル拡張および手動レコードマージ仕様
- コメント受信時の勝手な自動リスト追加や自動紐づけは一切行わない（完全手動管理）。
- `AvatarWindow` の「ニックネーム」管理タブ内の `m_usersTable` から不要な「管理ID」列を削除し、列構成を **`{"優先呼び名", "Twitch ID", "Discord 名", "愛称リスト", "操作"}`** (5列) とする。
- 配信主がテーブルの空欄セル（例: `Discord 名`）に対応するID（例: `alice_discord`）を手動入力・確定した際、同じIDを持つ別レコードが内部で自動検索される。
- 該当する別レコードが存在する場合、2つのレコードは1つのプロファイルに自動的にマージ（統合）され、重複行がクリアされる。
