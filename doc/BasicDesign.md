...
4. **ネットワーク監視：`TwitchReader Watchdog`（サイレント切断探知 ＆ バックグラウンド自動復旧エンジン）**
   - **問題背景**: TCP/WebSocket 接続において、ネットワーク瞬断やルーター切断等で「ソケットが物理的に破棄されても `disconnected` シグナルが発火しないサイレントドロップ（ゾンビ接続）」が発生する場合がある。
   - **Watchdog 監視構造**:
     - 定期タイマー（60秒周期）を駆動し、`m_lastDataReceivedTime`（最後に Twitch から PING やコメントデータを受信した時刻）を監視。
     - 無通信状態が 3分間 (180秒) 以上持続した場合、ソケットが固まっていると判断して古い WebSocket インスタンスを強制開放・破棄。
     - `wss://irc-ws.chat.twitch.tv:443` へバックグラウンドで即座に再接続・再認証・JOIN を実行し、ユーザーの手動操作なしで自動自己修復する。
