...
2. **`user_names.json` からの対応付け検索と呼びかけプロンプト構築**:
   - 発言のあったプラットフォーム（Twitch または Discord）に応じ、`users` オブジェクトから `twitch_id == userLower` または `discord_id == userLower` または プロファイルキーが一致するエントリを検索する。
...
3. **`m_usersTable` セル編集による自動マージ（統合）アルゴリズム**:
   - `AvatarWindow` の「ニックネーム」管理タブ内のテーブルで、配信主が空欄のセル（例: `Discord 名`）に対応するID（例: `alice_discord`）を入力した際、`AIClientManager::updateUserMapping` が発火する。
