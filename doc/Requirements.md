...
- **2種類の音声認識モードのサポート**:
  - **モード①（ウェイクワード起動モード）**: マイクを共有待機し、会話内で「アバター名（例: ぶるたろう）」が検出された際に自動的に音声認識をONにし、続く文言をAIへ自動送信して会話を開始する。

  - **モード②（プッシュ・トゥー・トーク [PTT] 方式）**: UIの「音声」ボタンを押している間だけマイク録音（`🎤 録音中...`）を行い、ボタンを離した瞬間に認識結果テキストを AI アシスタントへダイレクト送信する。
...
- **外部 HTTP (`/stt`) 経由の音声認識テキスト注入対応**:
  - HTTP サーバー（デフォルトポート 58082）に `/stt` および `/stt_input` エンドポイントを実装し、GET クエリパラメータ `?text=...` または JSON POST `{"text":"..."}` を受けて外部マシンやアプリから音声認識テキストを直接注入し、即座に AI 応答を自動起動する機能を提供する。
  - テキスト指定なしの GET `/stt` リクエストに対しては、サーバーが稼働中であることを示す正常ステータスレスポンス（`200 OK` JSON）を返却し、接続テストを可能とする。
