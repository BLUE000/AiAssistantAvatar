...
#### F-16-6: さくらAI事前Web検索ノイズ除去・Userロール最適化ダイレクト応答
- ユーザーが「さくらAI (`sakura`)」を優先プロバイダとして選択している状態で、天気（現在情報）、最新ニュース、株価・為替、Wikipedia的知識、日付依存の質問（「今日の○○」等）の Web 検索を必要とするクエリが入力された場合、さくらAI自身の `SakuraAIClient` が事前 Tavily 検索を実行する。
- 検索データから URL リスト、`[1]`, `[2]` 等の検索マークアップ、無関係な未来日付（週間天気等）や各種指数等のノイズを厳密にパージ・除去し、200〜300文字程度の極小・クリアな追加知識テキストへクレンジングする。
- 抽出された参考テキストを `system` ロールではなく **`user` ロールメッセージ (`{"role": "user", "content": "【参考情報】..."}`)** としてユーザー質問の直前に独立挿入して送信する。これによりさくらAIモデル (`llm-jp-3.1-8x13b-instruct4`) の誤発動ガード（拒否文言）を完全回避し、指定されたアバターキャラクター（「ぶるたろう」等）として直接回答を生成させる。
