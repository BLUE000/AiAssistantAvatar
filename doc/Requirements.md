...
#### F-16-6: さくらAI選択時限定 Manager AI Web検索代理回答
- ユーザーが「さくらAI (`sakura`)」を優先プロバイダとして選択している状態で、天気、為替、最新ニュース等の Web 検索を必要とするクエリが入力された場合、さくらAIモデル自身の安全ガードによる回答拒否を回避するため、`AIClientManager` は自動的かつサイレントに Manager AI（Groq 等）へ処理を代理移送する。
- Manager AI は Tavily 検索を実行した上で最新情報をパースし、指定されたアバターキャラクター（「ぶるたろう」等）としてユーザーへ回答を返却する。
- 本代理ルーティングは「さくらAI」選択時かつ検索必要時限定で適用され、他プロバイダ（Groq, OpenRouter, Mistral, Cerebras, HuggingFace）選択時は適用されず、各プロバイダのダイレクト通信仕様を維持する。
