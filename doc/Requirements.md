...
- AIを介さず、C++（システム側）で事前に定義されたルールに従って固定の返答を返す機能を提供する。
  ...
  - **応答ルールとトリガー**:
    ...
    - **使用中AI情報の問い合わせ**:
      - ユーザーからの入力プロンプトが「`aiは何か`」「`使用しているai`」「`動いているai`」「`どのai`」「`使っているai`」「`現在のai`」などに部分一致で合致した場合をトリガーとする。
      - トリガー検知時、モデル名やプランなどの詳細を除いた、稼働中のAI製品名（例:「`Mistral`」「`Groq`」「`Cerebras`」など）のみを応答テキスト（例:「`現在稼働しているAIは Groq です。`」）として即座に返却する。
