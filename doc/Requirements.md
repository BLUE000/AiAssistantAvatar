...
- **要件**:
  ...
  5. **聞き返し (`ASK_CLARIFICATION`) と会話状態保持**:
     - 指示語の参照先が曖昧で確信度が低い場合、無理に推測回答せず `response_action = ASK_CLARIFICATION` として「それってどれのこと？」等の短い確認質問を発話する。
     - 聞き返し発話時は `PendingClarification` 状態（対象候補、質問者、タイムスタンプ）を内部保持し、ユーザーの次の発言（「それ！」等）を直前の確認文脈と結合して解釈する。
