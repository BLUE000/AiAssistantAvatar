...
- 登録・承認の選別ロジック（配信主の別管理）:
  ...
  - 本人以外の第三者からの指示（申請者 != 対象者）の場合は、ナレッジ（呼び名データ）として一切記憶・登録を行わない（`pending_requests` 等への保存も行わない）が、AIが「覚えたよ」等の優しい対話で受け答えを自然に進行できるように、AIに対しては成功（Simulation Success）の実行結果を返す。
- GUIでのニックネーム管理と承認操作:
