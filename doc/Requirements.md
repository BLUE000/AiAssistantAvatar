...
- 会話履歴および対話コンテキストの管理をサポートする。
  - アプリケーションウィンドウ内に会話履歴表示領域を設け、履歴取得ボタンを押下した際に最新の対話履歴をマークダウン形式で整形して表示する機能を提供する。
...
### F-5: テキスト直接入力機能とコントロール統合UI
- マイクが使用できない環境や、音声入力が難しい場合のため、UIからキーボードで直接テキストを入力してAIに命令を送信する機能を提供する。
- 入力されたテキストは、音声認識（STT）を通さずに直接外部AI APIへのリクエストとして処理される。
- **ウィンドウ上に直接テキスト入力フォーム（QLineEdit）、送信・音声・⚙（メニュー）ボタンを配置し、右クリックメニューに依存せず一目で操作できる。**
- **右クリックメニューからは、不要となった「バリアント切り替え」および「アニメーション」の項目を削除し、メニューのシンプル化を図る。**

## 3. 非機能要件・アーキテクチャ制約
### N-1: GUIフレームワーク
- **Qt6 C++** を使用する。
- **ウィンドウのサイズを 750x480 に拡張し、左右2ペインの構成とする。**
  - **左側ペイン**: アバターの立ち絵表示、および直接テキスト入力フォーム、送信・音声・⚙ボタンを配置する。アバターに対応するテキストバルーンは、左側ペイン内のアバターの頭上に重なる位置に描画する。
  - **右側ペイン**: 会話履歴取得用のボタン、および取得結果をマークダウン形式で描画するテキスト表示領域（QTextBrowserなど）を配置する。
