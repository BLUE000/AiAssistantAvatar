...
## 2. 機能要件
### F-1: Twitchコメント取得・連携機能 (Authorization Code Flow)
- OAuth認可コードフロー (Authorization Code Flow) を用いてTwitchチャット（IRC over WebSocket）に接続する。
  - 認証に必要な「クライアントID」および「クライアントシークレット」は設定画面から入力・保存可能とする。
  - 初回認証時または再認証時には、認証URLに `force_verify=true` パラメータを付与してWebブラウザでTwitchの認可画面を開き、アカウント切り替えやログインの確認を毎回確実に行えるようにする。
  - ローカルに一時的に構築するリダイレクト待ち受け用HTTPサーバー（ポート番号は設定可能）で認可コード（Authorization Code）を受信し、それをもとにアクセストークンおよびリフレッシュトークンを自動的に取得してローカル設定に保存する。
  - トークンの有効期限切れ、またはチャット接続切断などの認証エラーを検知した際には、保存されているリフレッシュトークンを用いてサイレントにトークンの自動更新（リフレッシュ）を行い、ユーザーの再ログイン操作なしにチャット監視・返答を継続する。
