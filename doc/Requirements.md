...
- AIを介さず、C++（システム側）で事前に定義されたルールに従って固定の返答を返す機能を提供する。
  ...
  - **応答ルールとトリガー**:
    - 翻訳機能のトリガー（特定のコマンドや名前との連携）と同様に、単なるキーワードの部分一致ではなく、**名前（アバター名・呼称）とターゲットキーワードの複数AND条件**によってトリガーを判定し、無関係な雑談や他の対象への質問での誤検知（誤爆）を防止する。
    - **バージョン情報の問い合わせ**:
      - 「`version`」「`バージョン`」「`ばーじょん`」などの単体入力。
      - **または**、アバター名（例：「AIアシスタント」「アバター」やカスタム設定名）への言及があり、かつ「`バージョン`」「`version`」などのキーワードが同時に含まれる。
      - **かつ**、他の対象（マイクラ、Windowsなど無関係な名詞）を尋ねるものではないことを条件とする。
      - トリガー検知時、AIをバイパスして `version.h` の `PROJECT_VERSION` を用いた固定応答（例:「`現在のバージョンは v0.9.0 です。`」）を即座に返却する。
    - **使用中AI情報の問い合わせ**:
      - アバター名（「AIアシスタント」「アバター」など）への言及があり、かつ「`ai`」「`モデル`」「`プロバイダ`」のいずれかが同時に含まれる。
      - **かつ**、「`使っている`」「`使用している`」「`動いている`」「`稼働している`」などの稼働を示すワードや、所有格（「のAI」など）が同時に含まれる。
      - **かつ**、無関係な雑談や他社サービス（ChatGPTなど）に関するものではないことを条件とする。
      - トリガー検知時、モデル名やプランなどの詳細を除いた、稼働中のAI製品名（例:「`Mistral AI`」「`Groq`」「`Cerebras`」など）のみを応答テキスト（例:「`現在稼働しているAIは Groq です。`」）として即座に返却する。
