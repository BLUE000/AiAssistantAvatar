...
### F-33: 棒読みちゃん (Bouyomi-chan) 独立モジュール 読み上げ連携仕様 (`BouyomiChanClient`)
- **独立モジュール化 (`BouyomiChanClient`)**:
  ...
  - ポート 50001 指定時は 15 バイト制御ヘッダー付き TCP バイナリソケット通信、ポート 50080 指定時は HTTP GET 通信方式へ自動切り替えを行う。
- **設定ファイル管理 ＆ 自動補完仕様**:
  - 棒読みちゃんアクセス用 URL および有効化フラグを設定ファイル (`Config/local_settings.json`) の `"bouyomichan_enabled"` (初期値: `false`) および `"bouyomichan_url"` (初期値: `"http://localhost:50001"`) キーとして保存・管理する。
  - 設定ファイルにキーが存在しない場合、アプリケーション起動時に日本語説明コメント付きで該当キーを自動補完（自動追記保存）する。同PC上の固定パス運用だけでなく、別PCへの展開時にも IP アドレスを書き換えるだけで対応可能とする。
- **音声応答イベント連動 ＆ 送信処理**:
  - AI 応答イベント (`AIResponseReceived`) 受信時、応答ターゲットに `ReplyTarget::TTSVoice`（または音声入力・UI入力からの応答）が含まれる場合、回答本文テキストを指定された Bouyomi-chan ポート／プロトコルへ非同期送信して音声読み上げを実行する。



### F-3: 外部AI API連携機能
- 取得したテキストデータを外部AI APIに送信する。
  - 対応プロバイダ: Mistral AI、Groq、HuggingFace、OpenRouter、さくらAI (Sakura AI) をサポートする。
- AIから応答テキストを受信する。
- **ユーザー明示的選択プロバイダの独立通信・サイレントフォールバック禁止**:
  - ユーザーがUIの設定画面から明示的に特定プロバイダ（HuggingFace, OpenRouter, Sakura AI, Mistral, Groq等）を選択・指定している場合、エラー発生時にユーザーに無断で他のプロバイダへ裏で自動切り替え（サイレントフォールバック）を行ってはならない。
...
- **「レートリミットタブ」3 段階直感モニタリング ＆ 誤上限値補正機能**:
  ...
  - 過去のデータ等で Mistral の RPM 上限が `1` などに汚染されている場合は、起動時に正しい標準上限（Mistral/Groq: 30 RPM, HuggingFace/OpenRouter/Sakura: 60 RPM）へ自動補正する。
...
- **ウィンドウの左側ペインに `QTabWidget` を配置し、「チャット」タブ、「設定」タブ、「AI設定」タブなどを切り替えて利用可能とする。**
  ...
  - **AI設定タブ**: AIに関する設定値（各AIプロバイダ（Mistral / Groq / HuggingFace / OpenRouter / さくらAI）を選択する排他チェックボックス、各プロバイダのAPIキー、Tavily APIキー）をGUIから編集・適用可能とする。
  - **セキュリティ保護**: 秘密情報である各プロバイダのAPIキーおよび「Tavily APIキー」の入力欄は、パスワード入力モード（文字を隠すマスク表示）とする（※TwitchクライアントIDはUI上から削除され設定ファイルでのみ管理される）。
...
### F-7: ハイブリッドWeb検索機能（独立コンソールアプリ WebSearcher.exe 連携）
- AIが最新の時事情報や追加の知識を必要と判断した際、またはマネージャの事前タスク収集時に、独立したコンソールアプリケーション（`WebSearcher.exe`）をサブプロセス起動してWeb検索を実行し、回答に統合する機能をサポートする。
- **独立コンソールアプリケーション (`WebSearcher.exe`) 構成**:
  - Web検索モジュールをメインプロセスから分離し、独立した CLI ツールとして提供する。メインアプリは `QProcess` 等を用いてプロセス間実行・標準出力経由で結果を取得する。
  - **ハイブリッド検索エンジン構成**:
    - **DuckDuckGo (HTML版) スクレイピング**: APIキー不要かつ無制限に検索可能な基本エンジン。
    - **Tavily Search API**: 無料枠あり・高精度なAI検索エンジン。APIキーが引数で渡された場合に優先使用する。
  - **自動フォールバック ＆ タイムアウト制御**:
    - Tavily API キーが設定されている場合は Tavily を優先実行する。
    - Tavily API キー未設定、通信エラー（401/429/500等）、または一定時間（デフォルト 5000ms）アクセスできない（タイムアウト）場合は、自動的かつサイレントに DuckDuckGo 検索へフォールバックして処理を継続する。
    - 両系（Tavily および DuckDuckGo）ともに通信不可・失敗となった場合は、「Web検索不可: 検索結果を取得できませんでした。」を出力し、終了コード 1 を返却する。
  - **ノイズ除去・クレンジング**:
    - 取得した検索結果から URL リスト、HTMLタグ、マークダウン装飾、広告行を除去し、最大 10〜15 行のプレーンテキストを標準出力（stdout）に出力する。
...
- Twitchのコメントから、ユーザー自身のニックネーム指定（「〇〇と呼んで」「〇〇です」などの自己紹介・名乗り）や、他者へのニックネーム指定（「〇〇を✕✕と呼んで」など）を自動検知して処理する機能を提供する。
  - AI（Mistral等）の Function Calling（`update_nickname` ツール）を用いて、自己紹介を含む呼び名登録要求をパースし、申請者・対象者・ニックネームを抽出する。
  - **誤認防止（第三者・話題対象との区別）**:
    - 発言者が自身を対象として「〇〇と呼んで」「〇〇って呼んで」「私の名前は〇〇です」と明確に呼び名を指定・自己紹介した場合、または配信主が明示的に他者の呼び名を指定した場合にのみツール実行や登録対象とする。
    - 第三者・配信者・話題の対象として文脈中に名前が登場しただけの場合（例:「〇〇さんにおすすめの〜」「〇〇さんはどう思う？」等）は、ニックネーム登録対象と誤認してはならない。
  - **使い方案内**:
    - ユーザーからニックネーム機能の使い方や呼び名の設定方法について質問された場合は、Function Callingを不用意に実行せず、自然な対話で登録方法（「『〇〇と呼んで』と教えてくれたら設定するよ！」等）を分かりやすく案内する。
...
- `provider`: プロバイダ識別子（"groq" / "mistral" 等）
...
- **1. APIレスポンスヘッダー直接更新 (Primary)**:
  - APIレスポンスヘッダー（`x-ratelimit-remaining-requests`等）を含むプロバイダ（Groq, Mistral等）はヘッダー情報に基づき即時同期・更新する。
...
- Worker AIを優先度順（Groq > Mistral 等）に選択する。
...
- 天気（現在情報）、最新ニュース、株価・為替、Wikipedia的知識、日付依存の質問（「今日の○○」等）の各種検索クエリに対し、`AIClientManager` レイヤーにて独立コンソールアプリ `WebSearcher.exe` を通じた事前Web検索（Tavily優先 ➔ DuckDuckGoフォールバック）を一括実行する。
- 検索結果から URL リスト、`[1]`, `[2]` 等のマークアップ、各種ノイズ指数、無関係な未来日付等を一元的にクレンジング除去し、最長 12〜15 行（約 200〜300 文字）の短文テキストへスリム化する。
- スリム化された参考情報をユーザープロンプトの直前に `【参考情報】...\n\n[質問]` として一元結合し、全 AI プロバイダ（Groq, OpenRouter, Mistral, HuggingFace, Sakura 等）へ渡すことで、AIモデルの安全拒否ガードを 100% 回避して指定アバターキャラクターの言葉で直接応答させる。
...
- **情報取得ロジック (同名別人混入防止 ＆ Helix API 識別)**:
  ...
  - Twitch Helix API 呼び出し時は、`login` パラメータに英数字アカウントID（`msg-param-login`）を渡し、表示名（`msg-param-displayName`）と厳密に分離して 400 エラーを防止する。
- **トリガー条件**:
  - **レイド自動検知**: Twitchのレイドイベント（`USERNOTICE` raid）受信時に自動発動。
  - **手動コマンド**: `!so [ユーザー名]` や `/so [ユーザー名]`
  - **会話検知**: 設定が有効な場合、「[アバター名]、[ユーザー名]さんを紹介して」などの自然言語で発動。
- **レイド歓迎プロンプト文脈の厳格化（逆転誤認表現の防止）**:
  - 「相手クリエイターがリスナーを引き連れて当配信へ遊びに来てくれた（レイドしてくれた）」という状況を AI プロンプトに明確に指定する。
  - 相手配信者とリスナーへの感謝と歓迎を述べ、相手のチャンネル紹介を行う構成とし、「自分たちが今から相手の配信枠を見に行く（レイドしに行く）」ような逆転の誤認表現を完全に禁止する。
- **Twitch チャット欄への確実な自動コメント投稿**:
  - レイド受信時に入力ソースを `Twitch` として正しく設定し、生成されたお礼・紹介コメントを UI への表示・TTS 読み上げだけでなく、Twitch チャット欄へ 100% 確実に自動送信・中継する。
...
- **対応 AI プロバイダの拡張**:
  - 従来の Mistral, Groq に加え、オープンソース・多種 LLM アクセス・国内インフラに対応する **HuggingFace**, **OpenRouter**, **さくらAI (Sakura AI)** の 3 プロバイダを AI クライアントエンジンとして追加統合する。
...
- **機能要件**:
  ...
  3. **レートリミット利用可能量（最長残容量）基準の動的ルーティング**:
     - マネージャーは `RateLimitTracker` から登録済みの全プロバイダ（Mistral, Groq, HuggingFace, OpenRouter, Sakura）のリアルタイムステータスを取得。
...
- **概要**: 全対応プロバイダ（Sakura AI, Gemini, Groq, Mistral, HuggingFace, OpenRouter等）のレートリミット上限（RPM/RPD等）およびリアルタイム使用量を、ユーザーがクリック操作なしで一目監視できる「レートリミット」専用タブを新設する。これに伴い、「AI設定」タブ内に配置されていた旧「プロバイダ制限設定」フォームを全廃し、プロバイダごとの管理項目の不一致や将来の I/F 変更に対応可能な「動的項目モデル (`RateLimitQuotaItem`) ＆ UI完全動的生成」構造を導入する。
- **機能要件**:
...
### F-33: 汎用日替わりマクロ機能およびデフォルトナレッジ（おみくじ・星座占い）機能
- **概要**: ユーザーが C++ コードを修正することなく、Markdown ナレッジファイル内の記述のみで「ランダム抽選」や「日付・ユーザー単位の日替わり連動抽選」を制御できるようにするため、汎用マクロエンジン（`DailyTableSelect`, `DailyRandom`, `{Date}`, `{User}`）を提供する。また、デフォルトテンプレートとして独立フォルダ形式の「おみくじ」および「星座占い」ナレッジを標準同梱する。
- **機能要件**:
  1. **プレースホルダー自動展開 (`{Date}`, `{User}`)**:
     - `{Date}`: プロンプト評価時点のローカル日付（`YYYY-MM-DD` 形式）へ自動展開する。
     - `{User}`: 発言した視聴者・ユーザー名（Twitch/Discord/Direct）へ自動展開する（未指定時は空文字）。
  2. **汎用日替わりマクロ (`DailyRandom`, `DailyTableSelect`)**:
     - `DailyRandom(min, max, "seed")`: 指定されたシード文字列（日付やユーザー名等）を元にしたハッシュ値から、決定論的な疑似乱数（`min`〜`max`）を生成・置換する。
     - `DailyTableSelect("グループ", "テーブル", "カラム", "seed")`: 指定されたシード文字列のハッシュ値に基づき、対象テーブルから決定論的に 1 行を選択し、指定カラムのテキストへ置換する。
  3. **独立フォルダ形式デフォルトナレッジ (`Omikuji`, `Zodiac`)**:
     - `knowledge/Omikuji/`: `Ranks.md` (運勢リスト 7件) と `Details.md` (結果詳細プール 12件以上) を分離配置。
     - `knowledge/Zodiac/`: `Signs.md` (12星座リスト 12件) と `Results.md` (占い結果プール 14件以上) を分離配置。
     - ユーザーが各フォルダを `knowledge/` フォルダ外へ移動または削除するだけで、C++ を再コンパイルすることなく機能単体で安全に ON/OFF 切り替えできるものとする。

### F-34: Web検索・外部知識応答における情報量制御機能（3段階回答モード: SHORT / NORMAL / DETAILED）
- **概要**: Web検索等の調査が必要な質問に対し、内部的な調査・情報収集（Tavily/DDGによる検索やノイズ除去）は十分に実施しつつ、配信中の視聴者が読みやすいよう、ユーザー向けの最終回答長・情報量を「SHORT」「NORMAL」「DETAILED」の3段階で分離制御する二層構造を導入する。
- **機能要件**:
  1. **調査量（内部処理）と回答量（ユーザー出力）の完全分離**:
     - 回答モードの設定・判定によって Web 検索の実行有無や検索取得件数を低下させてはならない。
     - 内部では十分に調査・ファクト収集を行い、AI 生成プロンプト側の指示制御によりユーザー向け出力のみを適切に圧縮・調整する。
  2. **3段階回答モードの定義**:
     - **`SHORT` (デフォルト)**: 配信中に短時間で読める 1〜3 文程度。重要な理由・結論を最優先し、専門用語や細かい数値・背景解説を極力省いて平易かつ簡潔に出力する。
     - **`NORMAL`**: 数段落程度。「なぜそうなるのか」の理由や、必要な専門用語（平易な解説付き）・重要数値を適度に使用して説明する。
     - **`DETAILED`**: ユーザーが明示的に詳細解説を求めた場合。長文での詳細な背景解説、専門用語、詳細な数値・条件・例外事項を含めた包括的な解説を許可する。
  3. **自動意図判定 ＆ 会話コンテキスト連携（詳細化 ＆ 粒度縮小）**:
     - ユーザー入力に「詳しく」「詳細に」「もっと教えて」「仕組み」「専門的に」「具体的に」等の詳細化要求が含まれる場合は `NORMAL` または `DETAILED` として判定する。
     - 「簡単に」「一言で」「短く」「要点だけ」「ざっくり」等の簡潔化要求が含まれる場合、および指定がない通常質問の場合は `SHORT` を適用する。
     - **ユーザー指摘による粒度縮小・言い直し**: ユーザーから「細かい」「詳しすぎる」「長すぎる」「もっと短く」「要点だけでいい」などの指摘を受けた場合、即座に `SHORT` モードへ切り替え、直前の回答内容を 1〜2 文程度に圧縮・再要約して平易に言い直す。
     - **段階的詳細化**: 直前の会話で `SHORT` 応答を行った後に「もっと詳しく」等と質問された場合、過去の検索コンテキストを再利用しつつ `NORMAL` / `DETAILED` へ段階的に詳細化できるものとする。
  4. **既存機能との非干渉性**:
     - ローカル機能トリガー（おみくじ、星座占い等）、Web検索不要機能、AIルーティング、RAG、レートリミット管理に一切干渉せず、Web検索・知識質問の回答生成フェーズでのみ制約を注入する。

### F-35: 独立コンソールアプリ（CommunityObserver）による文脈違和感検知・対話促し機能
- **概要**: 単語単位の静的フィルター（第1層モデレーション）をすり抜けてしまう「普段の言動との急激な乖離」「小さな不満や愚痴の蓄積」「メンタル低下の兆候」を、発言者の過去ログや普段の傾向から独立して検知し、頭ごなしに遮断するのではなく優しく話を聞き出す対話誘導（傾聴ディレクティブ）を自動注入する見守り機能を提供する。
- **アーキテクチャ・機能要件**:
  1. **第1層静的フィルター（ScoreModerationEngine）との完全分離**:
     - 明らかな殺害予告、重度ヘイト、個人情報流出、違法行為等は第1層フィルターで即時ブロック（BLOCK）する。
     - 第1層フィルターを通過（SAFE判定）したすべての発言に対して、本機能が独立して動作する（「フィルター通過＝安全」と過信しない二重防衛構造）。
  2. **ログ収集・判定に特化した独立コンソールアプリ (`CommunityObserver.exe`)**:
     - 過去ログのファイル管理・蓄積（ログ収集）および違和感スコアリング（判定）を、独立した C++ コンソールアプリに分離する。
     - 本体（`AiAssistantAvatar.exe`）はチャット受信時に CLI サブプロセスを高速実行し、標準出力経由で JSON 判定結果を取得する。本体アプリ側に過去ログ解析やファイル管理の重いロジックを持たせず、障害を完全に隔離する。
  3. **3段階の文脈・違和感評価ステータス**:
     - **`Normal`**: 普段の発言傾向と一致しており、特段の違和感・過度な攻撃性がない通常状態。
     - **`DrasticChange`**: 普段は温和・冗談中心・ゲーム話題中心のユーザーから、突然特定対象への強い棘・否定・投げやりな発言が出た状態。
     - **`PersistentConcern`**: 特定人物や事象に対する不満・敵意が過去ログに複数回にわたり継続・蓄積している状態。
  4. **「否定・説教」ではなく「確認・開示（傾聴）」を優先する対話誘導**:
     - ログは「危険人物」等の人物評価ではなく、客観的な「発言履歴・日時・文脈」として扱い、AIが結論を決めつけない。
     - `DrasticChange` / `PersistentConcern` 検知時、Worker AI へ「かなり強い言い方だけど、何かあった？」「前も気にしてたみたいだけど、無理してない？」と話を開く方向へ促すシステムプロンプト指示を動的注入する。
  5. **常時ビルトイン稼働（UIトグル非設置）**:
     - ユーザーが「自分は大丈夫」と思い込んで誤って無効化してしまうリスクを防ぐため、設定画面でのON/OFFトグルは設けず、AIの標準的な対話・見守り能力として常時稼働させる。
- **非機能要件・安全免責制約**:
  1. **非医療・非専門的判断の制約**:
     - 本機能は医療機器や心理カウンセリング、危機介入サービスの代替ではなく、医療的・心理的な診断や断定は一切行わない。
  2. **動作範囲の制限**:
     - 本機能は、アプリが起動しており、かつウェイクワード、コマンドPrefix、または指定された配信チャット/Discordチャンネルで正常に受信・処理されたテキストの範囲内でのみ動作する。非監視下の通話や通信を網羅・保証するものではない。
  3. **ローカル完結・プライバシー保護**:
...
- **要件**:
  1. **Google Gemini API クライアントエンジン (`GeminiAIClient`)**:
     ...
     - **モデル設定仕様（自動最適化 ＆ 設定ファイルオーバーライド）**:
       - 無料利用枠（15 RPM / 1,500 RPD）および応答速度が最良となる `gemini-flash-latest` または動的探索モデルを使用し、404時は最新Flashモデルへ自動フォールバックする。
 ...
 ### F-40: 複数ユーザー会話における文脈判定・指示語・聞き返し対応機能

- **背景・目的**:
  - 複数ユーザーがチャットに参加する配信環境において、アバター名やウェイクワードでトリガーされた後、会話の文脈を誤解して不適切な回答を行う問題を解決する。
  - 具体的には、「〇〇さんが××って言ってるよ」（情報伝達）を質問として解説しない、「そこは▼▼だよ」（訂正）に対して人生論や励まし・一般論へ脱線せず誤りを素直に受け止める、「それ違うよ」（文脈不明な指示語）に対して無理に推測せず「それってどれのこと？」と短く聞き返す自然な対話能力を実現する。
- **要件**:
  1. **責務分担の分離**:
     - **ソフトウェア層 (C++)**: アバター名/ウェイクワード検知、発言者ID/発言時刻/メッセージIDのメタデータ管理、会話ログからの候補コンテキスト抽出、聞き返し状態（`PendingClarification`）の保持・復元、ルーティング制御。
     - **Manager AI (文脈判定層)**: 誰に向けた発言か（`target`）、会話行為の分類（`speech_act`）、指示語（これ・それ・あれ等）の参照先特定（`reference_message_id`）、確信度スコア（`reference_confidence`）、実行アクション（`response_action`）の判定。
     - **Worker AI (回答生成層)**: Manager AI の判定結果（Speech Act / Action）に応じたトーン・方針で最終回答文を生成。
  2. **会話ログからの候補コンテキスト抽出**:
     - 単なる「直前のAI発言」を固定的に使うのではなく、直近の会話ログから一定時間以内のAI参加発言、発言者の過去発言、AI名を含む発言等の関連候補を抽出して Manager AI に渡す。
  3. **会話行為 (Speech Act) の分類**:
     - 少なくとも以下の分類を判定する:
       - `INFORMATION` (情報伝達)
       - `CORRECTION` (過去発言の訂正・指摘)
       - `QUESTION` (質問)
       - `COMMAND` (指示・依頼)
       - `OPINION_DISAGREEMENT` (意見の相違)
       - `SUGGESTION` (提案)
       - `REACTION` (リアクション)
       - `OTHER` (その他)
  4. **訂正 (`CORRECTION`) に対する応答制御**:
     - AIの過去発言に対する訂正と判断された場合、Worker AI に対して一般論・人生論・励ましモード（「周りの意見に合わせて改善していく」等）を展開することを禁止し、素直に誤りを認めて「あ、〇〇なんだ！勘違いしてた、ごめん！」のように 1〜2 文で簡潔に返答させる。
  5. **聞き返し (`ASK_CLARIFICATION`) と会話状態保持・誤参照防止優先原則**:
     - **誤参照防止の最優先**: 「正解を無理に出すこと」よりも「誤った対象を参照して的外れ・不適切な回答をしないこと」を最優先とする。
     - **参照先特定不能の正常扱い**: 指示語の参照先を必ず 1 つに決定する必要はなく、会話ログから十分な確信を持って 1 つに特定できないことは正常な判定結果として扱う。
     - **聞き返し選択基準**: 候補が存在しない場合、または複数の候補があり確信を持って 1 つに絞り込めない場合は、無理に推測せず `reference_message_id = null`、`response_action = ASK_CLARIFICATION` として「それってどれのこと？」等の短い確認質問を発話する。
     - **状態保持**: 聞き返し発話時は `PendingClarification` 状態（対象候補、質問者、タイムスタンプ）を内部保持し、ユーザーの次の発言（「それ！」等）を直前の確認文脈と結合して解釈する。
   6. **不要な名乗り・自己紹介の抑止**:
      - 通常の会話・質問応答・雑談において、毎回「私は〇〇」「〇〇だよ」と自分の名前を名乗ったり自己紹介を挟むことを禁止する。自己紹介は初対面の挨拶や「名前は何？」「自己紹介して」と直接尋ねられた場合のみ行う。
   7. **質問に対する定型挨拶での誤魔化し防止**:
      - 「今日どのように過ごすの？」等の質問に対して、「今日も元気ですか？」「お手伝いがんばるよ」といった定型的な挨拶テンプレートを返さず、質問内容（過ごし方や予定など）に即してキャラクターらしく自然に回答する。
   8. **挨拶代行指示 (`COMMAND`) の文脈理解**:
...
- **背景・目的**:
  - AIプロバイダ（Groq, HuggingFace, OpenRouter, Gemini 等）のモデル名はプロバイダ側の世代交代やEOLによって頻繁に廃止・変更される。
  - UI上でモデル名を入力させる複雑さを排除し、APIキーのみの直感的なUIに統一するとともに、モデル設定を設定ファイル（`local_settings.json`）で一元管理する。
  - 空文字時や指定モデル廃止（HTTP 404）時に、アプリが自動で利用可能な最適モデルを探索・フォールバックして対話を継続する堅牢な自己修復機能を実現する。
- **要件**:
  1. **UI（AI設定タブ）のシンプル化**:
     - Manager AI（マネージャーAI）のモデル選択コンボボックスを撤去し、「有効チェックボックス」と「プロバイダ選択プルダウン」を同一行（同じ高さ）に横並びで配置（`有効: □　プロバイダ:[　　　　　　　▽]`）。
     - Groq, HuggingFace, OpenRouter, Gemini 等のモデル名入力欄（QLineEdit）をUIから撤去し、APIキー入力欄および有効化チェックボックスのみの洗練されたUIに統一する。
     - UIからの設定保存処理で、固定のモデル名文字列を上書き保存しない（設定ファイル内のユーザー指定値または空文字を維持する）。
  2. **モデル設定の一元管理**:
     - Worker AI および Manager AI のモデルは `local_settings.json` の各キー（`"manager_ai_model"`, `"groq_model"`, `"huggingface_model"`, `"openrouter_model"`, `"gemini_model"`）で管理する。
...
### F-48: Web検索結果ノイズ除去・テキストクレンジング ＆ 10秒タイムアウト最適化 ＆ 呼び名指示優先配置
- **検索結果テキストのサニタイズ・圧縮（ノイズ除去）**:
  - `TavilySearchProvider` および `DuckDuckGoSearchProvider` において、Web サイトから取得したテキストから不要なナビゲーション（「ホーム > ...」「ページを表示できませんでした」等）や無意味な年号・日付の選択肢テーブル、連続改行・空白を自動除去する。
  - 1 検索結果あたりの最大文字数を 350 文字、検索結果全体で最大 1,000 文字程度にスマートに切り詰め（スニペット化）、AI への入力トークンを最小化して推論速度向上とハルシネーション（誤読・固有名詞誤認）を防止する。
- **待機時間の 10 秒（10,000ms）最適化 ＆ タイムアウト時即時 Abort**:
  - `AIClientManager` のタイムアウトタイマーを 10,000ms（10秒）に最適化し、Function Calling（Web 検索）を伴う 2 往復推論を安全に完了可能とする。
