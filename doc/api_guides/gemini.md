...
Google Gemini（ジェミニ）は、Googleが提供する最先端のAIモデルです。Google AI Studioでは、高速かつ高精度な最新モデル（`gemini-flash-latest`, `gemini-2.5-flash` 等）が**1日あたり最大1,500リクエストの完全無料枠**で提供されています。
...
### ステップ④：キーのコピー
生成された API キー（`AIzaSy...` で始まる文字列）の横にあるコピーボタンをクリックしてコピーし、アプリの設定画面の **「Gemini (Google AI Studio)」** 欄に貼り付けます。
...
- **モデル**: 無料枠が最も広く（15 RPM / 1,500 RPD）最速応答の `gemini-flash-latest` が自動設定されます。（※手動で変更したい場合は設定ファイル `local_settings.json` の `"gemini_model"` を編集可能）
