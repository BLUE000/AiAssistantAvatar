...
| **UT-STT-05** | `CoreModule` 音声AI自動ルーティング (ウェイクワード検出) | 待機状態 (Idle) で `VoiceInputCompleted` (text: `"ぶるたろう、こんにちは"`) を `CoreModule::on_notify_events` に投入する。 | 1. アバター名が除去された `requestAI` (text: `"こんにちは"`) が発火すること。<br>2. 会話状態が Active に遷移し、無音タイマーが開始されること。 |
