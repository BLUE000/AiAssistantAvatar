...
| **UT-AI-05** | `AIClientManager` (ニックネーム他者擬似成功) | 申請者 bob、対象者 alice、ニックネーム「ありんこ」で `handleNicknameUpdateRequest` を実行する。 | 1. 戻り値が成功（`Success:`）であること。<br>2. `user_names.json` の `pending_requests` および `users` には bob からの申請は追加されず、記憶されないこと。 |
