...
| **UT-AI-05** | `AIClientManager` (ニックネーム他者拒否) | 申請者 bob、対象者 alice、ニックネーム「ありんこ」で `handleNicknameUpdateRequest` を実行する。 | 1. 戻り値がエラー・拒否（`Error:`）であること。<br>2. `user_names.json` の `pending_requests` リストは空のままであり、記憶されないこと。 |
